...
import { Loader2, Edit, Printer, AlertCircle, Mail, Icon } from "lucide-react";
...
const InvoiceDetail = () => {
  ...
  const handleUpdate = async (formData) => {
    try {
      ...
      toast.error("Failed to upadte the invoice");
      console.log(err);
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center bg-slate-50 rounded-lg">
        ...
        <p className="text-slate-500 mb-6 max-w-md">
          {" "}
          The invoice you are lookingh for does not exist or could not be loaded
        </p>
  ...
  return (
    <>
      ...
      <div id="invoice-content-wrapper">
        <div ref={invoiceRef} id="invoice-Preview" className="bg-white p-6 sm:p-8 md:p-12 rounded-lg shadow-md border border-slate-200 pb-[100vh]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 my-8">
            <div>
              ...
              <p className="font-semibold text-slate-800">{invoice.billFrom.buisnessName}</p>
              <p className="text-slate-800">{invoice.billFrom.address}</p>
              <p className="text-slate-800">{invoice.billFrom.email}</p>
              <p className="text-slate-800">{invoice.billFrom.phone}</p>
            </div>
            ...
            <table className="w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Qty</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Price</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Total</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-slate-200">
                {invoice.items.map((item,index)=> (
                  <tr key={index}>
                    ...
                    <td className="px-4 sm:px-6 py-4 text-sm font-medium text-slate-900">{item.quantity}</td>
                    <td className="px-4 sm:px-6 py-4 text-sm font-medium text-slate-900">{item.unitPrice.toFixed(2)}</td>
                    <td className="px-4 sm:px-6 py-4 text-sm font-medium text-slate-900">{item.total.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
